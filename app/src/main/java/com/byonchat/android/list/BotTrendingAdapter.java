...
/**
 * Created by Lukmanpryg on 9/2/2016.
 */
...
public class BotTrendingAdapter extends BaseAdapter implements Filterable
{
    ...
    public BotTrendingAdapter(Context ctx,ArrayList<ContactBot> contactBot,boolean showTitle) {
        context = ctx;
    ...
    public BotTrendingAdapter(Context ctx,ArrayList<ContactBot> contactBot,boolean showTitle,String b) {
        bold = b;
        context = ctx;
        this.showTitle = showTitle;
        this.catArrayList = contactBot;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (catArrayList.get(position).getLink().equalsIgnoreCase("") || catArrayList.get(position).getLink().equalsIgnoreCase("null")){
            Picasso.with(context).load(R.drawable.ic_room_door).networkPolicy(NetworkPolicy.NO_CACHE, NetworkPolicy.NO_STORE).into(holder.view);
        }else{
