...
public class ExceptionHandler implements
    ...
    //Detail Error with the whole log
    ...
    public void uncaughtException(Thread thread, Throwable exception) {
        ...
        File dir = new File(root.getAbsolutePath() + "/ByonChatLog/errorlog");
        if (!dir.exists()) {
            dir.mkdirs();
        }

        File file = new File(dir, "iss_log.txt");
    ...
    //Specific Log for easy understand error
    public void appendLog(String text) {
        File root = android.os.Environment.getExternalStorageDirectory();
        String currentDateTimeString = DateFormat.getDateTimeInstance().format(
                new Date());

        File logFile = new File(root.getAbsolutePath() + "/ByonChatLog/errorlog"+"/specificLog.txt");
        if (!logFile.exists()) {
            try {
