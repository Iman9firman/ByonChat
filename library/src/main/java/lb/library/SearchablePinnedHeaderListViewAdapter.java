...
public abstract class SearchablePinnedHeaderListViewAdapter<T> extends IndexedPinnedHeaderListViewAdapter implements
		...
		mFilter = new Filter() {
			...
			protected void publishResults(final CharSequence constraint, final FilterResults results) {
				...
				if (needRefresh){
					...
					final String[] generatedContactNames=generateContactNames(mFilterListCopy);
	...
	private String[] generateContactNames(ArrayList<T>  contacts)
	{
		final ArrayList<String> contactNames = new ArrayList<String>();
		if(contacts!=null)
			for(final T contactEntity : contacts){
						contactNames.add(contactEntity.toString());
