...
public class CircularContactView extends ViewSwitcher
  ...
  public CircularContactView(final Context context, final AttributeSet attrs)
    ...
    mContentSize=getResources().getDimensionPixelSize(R.dimen.margin50);
    if(isInEditMode())
  ...
  private void drawContent(final int viewWidth,final int viewHeight)
    ...
    if(mBackgroundColor!=0)
      {
        int round = mContentSize/3;
        float[] R = new float[] {round, round, round, round, round, round, round, round};
      roundedBackgroundDrawable=new ShapeDrawable(new RoundRectShape(R,null,null));
